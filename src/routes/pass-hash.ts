...
function core_sha1(x, len) {
	...
	var i = len >> 5;
	if (x[i] == undefined) x[i] = 0x80 << (24 - (len % 32));
	else x[i] |= 0x80 << (24 - (len % 32));
...
function str2binb(str) {
	...
	for (var i = 0; i < str.length * chrsz; i += chrsz) {
		...
		else bin[i >> 5] = (str.charCodeAt(i / chrsz) & mask) << (32 - chrsz - (i % 32));
	}
	/*for(var i = 0; i < str.length * chrsz; i += chrsz)
			bin[i>>5] |= (str.charCodeAt(i / chrsz) & mask) << (32 - chrsz - i%32);*/
	return bin;
...
function binb2b64(binarray) {
	...
	for (var i = 0; i < binarray.length * 4; i += 3) {
		/* SC - Get rid of warning */
		var b1 =
			binarray[i >> 2] != undefined ? ((binarray[i >> 2] >> (8 * (3 - (i % 4)))) & 0xff) << 16 : 0;
		var b2 =
			binarray[(i + 1) >> 2] != undefined
				? ((binarray[(i + 1) >> 2] >> (8 * (3 - ((i + 1) % 4)))) & 0xff) << 8
				: 0;
		var b3 =
			binarray[(i + 2) >> 2] != undefined
...
export var PassHashCommon = {
	...
	generateHashWord: function (
		...
		if (restrictDigits) s = PassHashCommon.convertToDigits(s, sum, hashWordSize);
		else {
			...
			if (restrictSpecial) s = PassHashCommon.removeSpecialCharacters(s, sum, hashWordSize);
		}
		// Trim it to size.
	...
	injectSpecialCharacter: function (sInput, offset, reserved, seed, lenOut, cStart, cNum) {
		...
		for (var i = 0; i < lenOut - reserved; i++) {
			...
			if (c >= cStart && c < cStart + cNum) return sInput; // Already present - nothing to do
		}
	...
	removeSpecialCharacters: function (sInput, seed, lenOut) {
		...
		while (i < lenOut) {
			...
			if (j < 0) break;
			if (j > 0) s += sInput.substring(i, i + j);
			s += String.fromCharCode(((seed + i) % 26) + 65);
			i += j + 1;
		}
		if (i < sInput.length) s += sInput.substring(i);
		return s;
	},
